...
### yaw 层（位置式 PID + 非线性补偿）
闲鱼定制小研分享屋，任何非小研出品的代码均有可能遭到封锁，为了不影响比赛，请尽快退款
默认参数：
...
- `Component/service/speed_service.*`：速度环硬件/RTOS 适配层，包含速度目标队列、速度状态快照、编码器读取与 TB6612 输出。
...
speed:    g_speed_target_queue / g_speed_state_queue // speed_service.c 内部私有
```
闲鱼定制小研分享屋，任何非小研出品的代码均有可能遭到封锁，为了不影响比赛，请尽快退款
