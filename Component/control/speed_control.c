...
static void speed_control_publish_output(const speed_loop_core_output_t *output)
{
...
static void speed_control_publish_snapshot(void)
{
    speed_loop_core_output_t output = {0};
    speed_loop_core_get_output(&g_speed_core, &output);
    speed_control_publish_output(&output);
}

static void speed_control_apply_motor_output(const speed_loop_core_output_t *output)
{
    if (output == NULL) {
        return;
    }
...
bool speed_control_init(void)
{
    ...
    encoder_reset();
    speed_control_publish_snapshot();
    return true;
}
...
void speed_control_step_10ms(void)
{
    ...
    if (speed_loop_core_update(&g_speed_core,
                               ...
                               &output)) {
        speed_control_apply_motor_output(&output);
    }
